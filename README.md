...
We change the turning method in this challenge because the previous one has difficulty working consistently with the number of other turns we do to avoid the obstacles in this challenge. When the robot is on an angle, usually while turning, the previous method can trigger when it isn't supposed to.
