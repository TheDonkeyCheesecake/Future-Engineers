...
- `video` - [YouTube link](https://www.youtube.com/your_video_link) of our robot running each program on the game field
...
![Team Members](team_members_image.png)
...
![Robot Image 1](robot_image_1.png)
![Robot Image 2](robot_image_2.png)
...
Our Raspberry Pi setup involves downloading the Raspberry Pi OS, writing it to a microSD card using Raspberry Pi Imager [Raspberry Pi Imager](https://www.raspberrypi.com/software/), and configuring SSH and Wi-Fi. After inserting the SD card, powering up the Pi, and identifying its IP address on the network, we can remotely access it via PuTTY [PuTTY](https://www.putty.org) for SSH and VNC Viewer [VNC Viewer](https://www.realvnc.com/en/connect/download/viewer/) for remote desktop control.
...
The obstacle challenge brings the addition of one more segment of code, being the obstacle avoidance, along with a change of the turning algorithm.
...
We change the turning method in this challenge because the previous one has difficulty working consistently with the number of other turns we do to avoid the obstacles in this challenge. When the robot is on an angle, usually while turning, the previous method can trigger when is isn't supposed to.
